...
- **Domain vocabulary matters.** Use STRIDE categories, CVSS v4.0 vectors, IEC 62443 terminology, and the 5x5 Likelihood x Impact risk matrix with CIA + Safety impact dimensions consistently with existing knowledge-base entries and docs — this is TRA/security-engineering domain content, not generic app data. In particular, an inherent limitation of a communication protocol (e.g. no source authentication in Modbus RTU) is not automatically a vulnerability of the component using it — see [.ai/README.md](../.ai/README.md)'s "Protocol limitations vs. product vulnerabilities" section and `Threat.classification`/`responsibility` in `webapp/client/src/types.ts`.
- **AI-assisted TRA workflow.** For interviewing users, reviewing, or sparring on TRA content itself (as opposed to writing tool code), use the agents in [.github/agents/](agents/) and prompts in [.github/prompts/](prompts/) — see [.ai/README.md](../.ai/README.md) for the full framework, including the `.tra-knowledge.json` per-project sidecar file and the default write-immediately policy for TRA JSON writes (git provides the audit/revert path; the user can ask for pre-write review instead).
