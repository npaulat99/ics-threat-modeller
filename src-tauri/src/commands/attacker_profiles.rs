...
pub fn create_attacker_profile(
    ...
    conn.execute(
        "INSERT INTO attacker_profiles (id, project_id, name, skill_level, access_level, description, tag, is_active, created_at, updated_at)
...
pub fn list_attacker_profiles(
    ...
    let mut stmt = conn
        .prepare(
            "SELECT id, project_id, name, skill_level, access_level, description, tag, created_at, updated_at, is_active
...
pub fn update_attacker_profile(
    ...
    if let Some(v) = tag {
        sets.push(format!("tag = ?{}", param_idx));
        params_vec.push(Box::new(v));
        param_idx += 1;
    }
