#!/usr/bin/env node
// Structural acceptance check for the TRA agent framework: proves the approval gate is wired
// (only the Facilitator can write files) and documented, and that the one-question-at-a-time
// interview rule is present. This exercises the framework's *mechanism*, not the full TRA domain
// model or a live model run — mirrors the plain-assert style of tools/test-models.js.
//
...
function testOnlyFacilitatorCanEdit() {
    ...
    for (const name of READ_ONLY_AGENTS) {
        ...
        assert.ok(!tools.includes('edit'), `${name} must stay read-only (no 'edit' tool) — the approval gate relies on every other agent being unable to write files.`);
    }
}

function testApprovalGateIsDocumented() {
    const body = readAgent(FACILITATOR);
    assert.match(body, /explicit approval/i, 'tra-facilitator instructions must require explicit approval before writing TRA JSON.');
    assert.match(body, /no exception/i, 'tra-facilitator instructions must state the approval gate has no exception.');

    const instructions = readFileSync(join(instructionsDir, 'tra-json-editing.instructions.md'), 'utf8');
    assert.match(
        instructions,
        /Never write to any of these files without the user having explicitly approved/i,
        'tra-json-editing.instructions.md must restate the approval gate in the same terms as the Facilitator.',
    );
}
...
const tests = [testOnlyFacilitatorCanEdit, testApprovalGateIsDocumented, testOneQuestionAtATime, testTrustBoundaryAliasExceptionIsConsistent];
